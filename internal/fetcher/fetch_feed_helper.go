...
// Store fetched feed items(posts) into db
func StoreFeedItems(ctx context.Context, queries *database.Queries, fetchedFeed *gofeed.Feed, feedID uuid.UUID) error {
	for _, item := range fetchedFeed.Items {
		...
		if err := queries.InsertItem(ctx, database.InsertItemParams{
			FeedID: feedID,
			Guid:   guid,
			Url:    item.Link,
			Title:  item.Title,
